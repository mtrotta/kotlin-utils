...
class TargetCalendar : WorkingCalendar {
    ...
    companion object {
        ...
        private fun isWorkingDay(calendar: Calendar): Boolean {
            val targetCalendar = getTargetCalendar(calendar.get(Calendar.YEAR))
        ...
        private fun getTargetCalendar(vararg years: Int): Set<Calendar>? {
            ...
            return if (targetCalendar.isEmpty()) null else targetCalendar
        }

        private fun isEaster(date: Date): Boolean {
            val calendar = GregorianCalendar()
            calendar.time = date

            val year = calendar.get(Calendar.YEAR)
            val dateYMD = year * 10000 + calendar.get(Calendar.MONTH) * 100 + calendar.get(Calendar.DAY_OF_MONTH)
            val easter = findEaster(year)
            calendar.time = easter
            val easterYMD = year * 10000 + calendar.get(Calendar.MONTH) * 100 + calendar.get(Calendar.DAY_OF_MONTH)
